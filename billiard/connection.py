...
class _ConnectionBase(object):
    ...
    def __init__(self, handle, readable=True, writable=True):
        if (type(handle) is _ConnectionBase._SocketContainer):
          self._socket = handle.sock # keep it around to avoid getting deleted
...
def detach(sock):
  if hasattr(sock, 'detach'):
    return sock.detach()
  # older socket lib does not have detach.  We'll keep a reference around so
  # that it does not get garbage collected
  return _ConnectionBase._SocketContainer(sock)
